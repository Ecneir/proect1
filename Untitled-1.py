...
class MenuScreen(Screen):
    def __init__(self, **kw):
        ...
        box.add_widget(Button(text='Дневник питания',  background_color=(10,10,0,1), color=(0,0,0,1), on_press=lambda x:
...
class SortedListFood(Screen):
    ...
    def on_enter(self):  # Будет вызвана в момент открытия экрана
        ...
        for f, d in sorted(dic_foods.items(), key=lambda x: x[1]):
            fd = f.decode('u8') + ' ' +str(d)  # (datetime.fromtimestamp(d).strftime('%Y-%m-%d'))
            btn = Button(text=fd, size_hint_y=None, height=dp(40), background_color=(10,10,0,1), color=(0,0,0,1))
            self.layout.add_widget(btn)
            callories+=d
        f11="Общее кол-во калорий  "+str(callories)
        self.f1 = Label(text=f11)
        self.layout.add_widget(self.f1)
...
class AddFood(Screen):
    ...
    def buttonClicked(self, btn1):
        ...
        try:
            self.app.user_data[self.txt1.text.encode('u8')] =int( self.txt2.text)
        except:
            self.app.user_data[self.txt1.text.encode('u8')] =0
    ...
    def __init__(self, **kw):
        ...
        self.txt2 = TextInput(text='', multiline=False, height=dp(40),
                              size_hint_y=None, hint_text="калораж")
        box.add_widget(self.txt1)
        box.add_widget(self.txt2)
        btn1 = Button(text="Добавить блюдо", background_color=(10,10,0,1), color=(0,0,0,1), size_hint_y=None, height=dp(40))
